...
func main() {
	...
	q, err := ch.QueueDeclare(
		"task_queue", // name
		true,         // durable <-- diff, tetap ada if server down
	...
	// utk memberikan sinyal tidak menambah pesan lebih dari 1
	// jika belum ack-ed
	err = ch.Qos(
		1,     // prefetch count
		0,     // prefetch size
	...
	msgs, err := ch.Consume(
		q.Name, // queue
		"",     // consumer
		false,  // auto-ack <-- diff, pengakuan otomatis pesan diterima
		false,  // exclusive
		false,  // no-local
		false,  // no-wait
		nil,    // args
